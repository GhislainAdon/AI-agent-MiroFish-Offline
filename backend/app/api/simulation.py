"""
Routes API liees a la simulation.
Etape 2: lecture et filtrage des entites, preparation OASIS et execution automatisee.
"""
...
# Prefixe d'optimisation des prompts d'entretien.
# Il force les agents a repondre directement en texte au lieu d'appeler des outils.
...
def optimize_interview_prompt(prompt: str) -> str:
    """
    Optimiser les questions d'entretien en ajoutant un prefixe qui evite les appels d'outils.
...
# ============== Interface de lecture des entites ==============
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    """
    Verifier si la simulation est prete.
...
def prepare_simulation():
    """
    Preparer l'environnement de simulation avec une tache asynchrone et une configuration generee par LLM.

    Operation longue: l'interface retourne immediatement un task_id.
    Utiliser /api/simulation/prepare/status pour suivre la progression.

    Fonctionnalites:
    - Detecte les preparations deja terminees pour eviter les doublons.
    - Retourne directement les resultats existants si la preparation est deja faite.
    - Supporte la regeneration forcee avec force_regenerate=true.

    Etapes:
    1. Verifier si la preparation est deja terminee.
    2. Lire et filtrer les entites du graphe de connaissances.
    3. Generer un profil agent OASIS pour chaque entite.
    4. Generer la configuration OASIS via LLM.
...
# ============== Interface de controle de l'execution de simulation ==============
